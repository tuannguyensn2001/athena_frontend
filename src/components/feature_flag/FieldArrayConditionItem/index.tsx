...
export function FieldArrayConditionItem({ index, onRemove, id: _id }: Prop) {
    const { control, watch, resetField } = useFormContext<TargetGroupForm>();
    ...
    useEffect(() => {
        resetField(`conditions.${index}.condition_id`, {
            defaultValue: 0,
        });
    }, [targetObjectId, index]);
    ...
    const { data } = useQuery<AppResponse<ICondition[]>, ApiError>({
        ...
        queryFn: async () => {
            const response = await ApiFeatureFlag.get(
                `/api/v1/conditions/target_object/${targetObjectId}`,
            );
    ...
    const { data: dataOperators } = useQuery<
        AppResponse<IOperator[]>,
        ApiError
    >({
        queryKey: ['operators', conditionId],
        queryFn: async () => {
            const response = await ApiFeatureFlag.get(
                `/api/v1/operators/condition/${conditionId}`,
            );
            return response.data;
        },
        staleTime: Infinity,
